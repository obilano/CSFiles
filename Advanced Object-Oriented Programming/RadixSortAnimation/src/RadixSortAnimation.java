 ...
 * Assignment:	Radix Sort Animation
...
public class RadixSortAnimation extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Scene scene = new Scene(borderPane, 775, 360);
        primaryStage.setTitle("Radix Sort Animation"); // Set the stage
        // title
    ...
    /**
     * Return the digit at the specified position. The last digit's position is
     * 0.
     */
    ...
    class AnimationPane extends Pane {
        private int startingX = 30;
        private int startingY = 20;
        private int boxWidth = 35;
        ...
        protected void repaint() {
            ...
            for (int i = 0; i < bucket.length; i++) {
                ...
                if (i == key && bucket[i].size() > 0) {
                    Text text = new Text(x + 10 + 5, y + bucket[i].size() * 20,
                            bucket[i].get(bucket[i].size() - 1) + "");
                    text.setFill(Color.RED);
                    this.getChildren().add(text);
                }
